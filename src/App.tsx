          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-8 border-b border-gray-300 dark:border-white/10">
            {[nextOne, nextTwo, nextThree]
              ...
              .map((program) => (
                <button
                  key={
                    (program as Program).id ||
                    (program as Program).title
                  }
                  onClick={() =>
                    onNavigateToProgram(program as Program)
                  }
...
const ProgramPage = ({
  ...
  const presenterImage = getProgramImage(program)
  const progress = getProgramProgress(program)
  const nextPrograms = queue.slice(0, 3)
  const size = 190
  const strokeWidth = 6
  const radius = (size - strokeWidth) / 2
  const circumference = 2 * Math.PI * radius
    ...
    <section className="bg-white dark:bg-[#121212] text-gray-950 dark:text-white">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-10">
        <div className="flex items-center justify-between gap-4 mb-8">
          <button
            onClick={onBack}
            className="rounded-full border border-gray-300 dark:border-white/10 px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-200 hover:border-orange-500 hover:text-orange-500 transition"
          >
            Back
          </button>
          ...
          <button
            ...
            className="rounded-full bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 text-sm font-medium transition"
          >
            View schedule
          </button>
        </div>
        ...
        <div className="flex flex-col md:grid md:grid-cols-[220px_1fr] gap-8 md:gap-10 items-center border-b border-gray-300 dark:border-white/10 pb-8 md:pb-10">
          ...
          <div className="text-center md:text-left w-full">
            <div className="flex items-center justify-center md:justify-start gap-2 text-sm mb-2">
              <span className="font-black text-orange-500">PROGRAM</span>
              <span className="text-gray-500">·</span>
              <span className="text-gray-500">
                {formatRangeToAmPm(
                  program.startTime,
            ...
            <p className="mt-2 text-base md:text-lg text-gray-700 dark:text-gray-300">
              {program.description ||
                'Listen live to Praise FM — Christian music and worship.'}
            </p>

            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {program.host || program.presenter || ''}
            </p>
            ...
            <button
              onClick={onListenClick}
              className="mt-6 bg-orange-500 hover:bg-orange-600 text-white px-10 md:px-12 py-3 md:py-4 font-black text-lg transition active:scale-95 inline-flex items-center justify-center gap-3 rounded-xl"
            >
              {isPlaying ? (
                <Pause size={22} />
              ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-8 border-b border-gray-300 dark:border-white/10">
          {nextPrograms.map((nextProgram) => (
            <div
              key={nextProgram.id || nextProgram.title}
              className="bg-gray-100 dark:bg-[#1A1A1A] p-4 rounded-2xl"
            >
              <p className="text-[11px] font-black text-orange-500 uppercase tracking-wide mb-0.5">
                {formatRangeToAmPm(
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate">
                {nextProgram.host || nextProgram.presenter}
              </p>
            </div>
          ))}
        </div>
...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    audio.volume = parseFloat(
      localStorage.getItem('praise-volume') || '0.8'
    )
  ...
  const seo = {
    title:
      'Praise FM USA - 24/7 Worship & Gospel Radio',
    description:
