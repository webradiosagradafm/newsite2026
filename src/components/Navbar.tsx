...
const Navbar: React.FC<NavbarProps> = ({ activeTab, theme, onToggleTheme }) => {
  ...
  return (
    <header className="bg-white dark:bg-[#000] border-b border-gray-100 dark:border-white/5 sticky top-0 z-50 transition-colors duration-300">
        ...
        <div className="flex items-center h-full space-x-12">
          ...
          <nav className="hidden md:flex items-center space-x-8 h-full">
            ...
            <button 
              ...
              className={`flex items-center space-x-2 text-[15px] font-medium transition-all h-full border-b-2 px-1 uppercase tracking-tighter ${
                activeTab === 'my-sounds' ? 'text-black dark:text-white border-[#ff6600]' : 'text-gray-500 border-transparent hover:text-black dark:hover:text-white'
              }`}
            >
